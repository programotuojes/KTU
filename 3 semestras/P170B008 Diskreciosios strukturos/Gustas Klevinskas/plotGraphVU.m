function [Vkor] = plotGraphVU(V,U,orgraf,arc,Vkor,poz,Fontsize,lstor,spalva,kelioPabaiga)
  ...
  if ~arc   % tiesios briaunos braizymas  (arc = 0)
      ...
      s = size(a);  sv = [];  if s(2)>2, sv = a(3);  end     % lanko svoris
              if ~isempty(sv)   % uzrasomas briaunos svoris
                  v = v1 + 0.4*(v2-v1);  ll = 1.75*r;  hh = 1.5*r;
...
% kelia paduodamos tik 2 koordinates
if length(Vkor) > 2
