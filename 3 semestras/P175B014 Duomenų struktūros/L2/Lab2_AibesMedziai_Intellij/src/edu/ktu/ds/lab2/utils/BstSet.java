...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    // Metodas nr. 1 (pasirodo nereikėjo)
    public void addAll(BstSet<? extends E> c) {
     ...
     *
     * @param from - pradinis aibės poaibio elementas.
     * @param to   - galinis aibės poaibio elementas.
     * @return Grąžinamas aibės poaibis nuo elemento element1 iki element2.
     */
    @Override
    public Set<E> subSet(E from, E to) {
        if (from == null || to == null) {
            throw new IllegalArgumentException("Element is null in subSet(E from, E to)");
        }
    ...
    private void subSetRecursive(Set<E> newList, E from, E to, BstNode<E> node) {
        if (node != null) {
            if ((c.compare(from, node.element) <= 0) && (c.compare(to, node.element) > 0))
                newList.add(node.element);
